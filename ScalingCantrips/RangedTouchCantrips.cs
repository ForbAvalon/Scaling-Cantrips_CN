    ...
    static class PatchUnholy
    {
      ...
      static void AddFirebolt()
      {
        ...
        var UnholyZap = Helpers.CreateBlueprint<BlueprintAbility>("RMFirebolt", bp =>
        {
          ...
          bp.AddComponent(
            Helpers.Create<SpellDescriptorComponent>(
              c =>
              {
                c.Descriptor =
                  new SpellDescriptorWrapper()
                  {
                    m_IntValue = (long)SpellDescriptor.Fire
                  };
              }));
          bp.AddComponent(
            Helpers.Create<AbilityDeliverProjectile>(
              c =>
              {
                c.m_Projectiles =
                  new BlueprintProjectileReference[]
                  {
          ...
          RankConfig.m_CustomProperty =
            CantripPatcher.BlueprintPatcher.CreateHighestCasterLevel().ToReference<BlueprintUnitPropertyReference>();
          bp.AddComponent(RankConfig);
          bp.AddComponent(Helpers.Create<CantripComponent>());
          bp.AddComponent(
            Helpers.Create<AbilityEffectRunAction>(
              c =>
              {
                c.Actions =
                  ...
                  {
                    Actions =
                    new Kingmaker.ElementsSystem.GameAction[]
                    {
      ...
      static void AddSpellToArcaneCantrips(BlueprintAbility bp)
      {
        ...
        var CantripsList =
          ...
          {
            EldritchScionCantrips,
            EldritchScoundrelCantrips,
            MagusCantrips,
            WizardCantrips,
            SorcererCantrips,
            EmpSorcCantrips,
        ...
        foreach (var cantrip in CantripsList)
        {
          ...
          cantrip.GetComponent<AddFacts>().m_Facts =
            cantrip.GetComponent<AddFacts>().m_Facts.AppendToArray(bp.ToReference<BlueprintUnitFactReference>());
          cantrip.GetComponent<LearnSpells>().m_Spells =
            cantrip.GetComponent<LearnSpells>().m_Spells.AppendToArray(bp.ToReference<BlueprintAbilityReference>());
          cantrip.GetComponent<BindAbilitiesToClass>().m_Abilites =
            cantrip.GetComponent<BindAbilitiesToClass>().m_Abilites.AppendToArray(bp.ToReference<BlueprintAbilityReference>());
        }
      ...
      static void AddSpellToThasEvocation(BlueprintAbility bp) //this works weirdly and needs to be done for firebolt
      {
        ...
        var CantripsList =
          new BlueprintFeature[] { IllusCantrips, TransCantrips, NecroCantrips, WizardCantrips, EnchantCantrips };
